    ...
    FIFO_BUFFER const *b)
{
	  ...
	  bool ret = (b ? (FIFO_Count(b) == b->buffer_len) : true);
...
{
		fifoENTER_CRITICAL_FROM_ISR();
	  bool ret = (b ? (count <= (b->buffer_len - FIFO_Count(b))) : false);
		fifoEXIT_CRITICAL_FROM_ISR();
    return ret;
}
...
{
	 fifoENTER_CRITICAL_FROM_ISR();
	 unsigned head = b->head % b->buffer_len;
	 unsigned tail = b->tail % b->buffer_len;
	 
	 if(head != tail)		/*turn around*/
	 {
		  if(head < tail)
			{
...
{
		fifoENTER_CRITICAL_FROM_ISR();
	  bool ret = (b ? (FIFO_Count(b) == 0) : true);
		fifoEXIT_CRITICAL_FROM_ISR();
    return ret;
}
